...
================================================
  ...
  Run from Command Prompt:
      python emotion_svm.py --install   (first time only)
      python emotion_svm.py --train
      python emotion_svm.py --detect
      python emotion_svm.py --evaluate

  Folder structure required:
      Ai-Assignment/
      ├── emotion_svm_rafdb.py   ← this file
      ├── RAF-DB/
      │   └── train/
      │       ├── angry/
      │       ├── disgust/
...
# ─────────────────────────────────────────────
DATA_DIR  = r'C:\Users\User\Desktop\AiAssignment\Ai-Assignment\RAF-DB\train'
MODEL_DIR = 'models'
IMG_SIZE  = (64, 64)
...
def install_dependencies():
    ...
    print("[INSTALL] Done. Now run:  python emotion_svm.py --train\n")
...
# ─────────────────────────────────────────────
#  STEP 5 — Evaluate + save plots to Desktop
# ─────────────────────────────────────────────
def evaluate_and_save(svm, scaler, pca, le, X_test, y_test):
    ...
    print(f"\n[4/5] Evaluating model and saving to: {desktop_path}")
    ...
    sns.heatmap(cm, annot=True, fmt='d', xticklabels=EMOTIONS, 
                yticklabels=EMOTIONS, cmap='Blues', ax=axes[0])
    ...
    for e in EMOTIONS:
        mask = y_true_lbl == e
        a = (y_pred_lbl[mask] == e).mean() if mask.sum() > 0 else 0
        per_class_acc.append(a)

    axes[1].bar(EMOTIONS, [a * 100 for a in per_class_acc], color='steelblue')
    axes[1].axhline(acc * 100, color='red', linestyle='--', label=f'Overall {acc:.1%}')
    ...
    plt.tight_layout()
    # SAVE TO DESKTOP
    plt.savefig(os.path.join(desktop_path, 'confusion_matrix.png'), dpi=150)
    plt.close()

    # ── Learning curve (safe mode) ────────────────────────────
    from sklearn.model_selection import learning_curve
    train_sizes, train_scores, val_scores = learning_curve(
        svm, X_test, y_test, cv=2, n_jobs=1, train_sizes=np.linspace(0.3, 1.0, 4)
    )
    ...
    plt.legend()
    # SAVE TO DESKTOP
    plt.savefig(os.path.join(desktop_path, 'learning_curve.png'), dpi=150)
    plt.close()

    # ── Metrics Text ──────────────────────────────────────────
    with open(os.path.join(desktop_path, 'metrics.txt'), 'w') as f:
...
# ─────────────────────────────────────────────
#  STEP 6 — Save model to Desktop
# ─────────────────────────────────────────────
def save_models(svm, scaler, pca, le):
    import joblib
    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")

    print("\n[5/5] Saving model to Desktop...")
    bundle = {
        'svm'     : svm,
        'scaler'  : scaler,
        'pca'     : pca,
    ...
    path = os.path.join(desktop_path, 'emotion_model.pkl')
...
def print_help():
    print("""
Usage:
  python emotion_svm.py --install    Install all packages (run once)
  python emotion_svm.py --train      Train SVM on RAF-DB
  python emotion_svm.py --detect     Real-time webcam detection
  python emotion_svm.py --evaluate   Re-evaluate saved model
